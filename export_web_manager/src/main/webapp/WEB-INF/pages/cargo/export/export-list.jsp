...
<script>
    ...
    function packages() {
        var id = getCheckId123()
        var  str=""
        if(id) {
            for (var i = 0; i < id.length; i++) {
                 str+=(id[i]+",")
            }
            $.post("/cargo/packing/state.do","id="+str,function (d) {
                  if (d==1){
                      location.href="${ctx}/cargo/packing/toAdd.do?id="+str;
                  } else {
                      alert("只能提交:::状态为已报运的，其他不行!")
                  }
                ...
                <div class="pull-left">
                    <div class="form-group form-inline">
                        <div class="btn-group">
                            ...
                            <button type="button" class="btn btn-default" title="装箱" onclick='packages()'><i class="fa fa-file-o"></i> 装箱</button>
                        </div>
                    </div>
                </div>
